...
def KMM(X,Xtest,_A=None, _sigma=1e1,beta=0.2):
    ...
    H = torch.exp(- 1e0 * utils.pairwise_distances(X)) + torch.exp(- 1e-1 * utils.pairwise_distances(X)) + torch.exp(- 1e-3 * utils.pairwise_distances(X))
    f = torch.exp(- 1e0 * utils.pairwise_distances(X, Xtest)) + torch.exp(- 1e-1 * utils.pairwise_distances(X, Xtest)) + torch.exp(- 1e-3 * utils.pairwise_distances(X, Xtest))
    z = torch.exp(- 1e0 * utils.pairwise_distances(Xtest, Xtest)) + torch.exp(- 1e-1 * utils.pairwise_distances(Xtest, Xtest)) + torch.exp(- 1e-3 * utils.pairwise_distances(Xtest, Xtest))
    H /= 3
    f /= 3
